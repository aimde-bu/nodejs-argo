...
const SUB_PATH = process.env.SUB_PATH || 'db';       // 订阅路径
const PORT = process.env.SERVER_PORT || process.env.PORT || 3000;        // http服务订阅端口
const UUID = process.env.UUID || '9d65e362-9ca6-4b5b-a04f-bfb31be30882'; // 使用哪吒v1,在不同的平台运行需修改UUID,否则会覆盖
...
const NGROK_AUTHTOKEN = process.env.NGROK_AUTHTOKEN || ''pushchair-aspirate-playable.ngrok-free.dev;  // Ngrok authtoken, 配置后使用 ngrok 替代 cloudflared
const NGROK_DOMAIN = process.env.NGROK_DOMAIN || '';        // Ngrok 固定域名 (需付费计划), 留空使用随机域名
const NGROK_REGION = process.env.NGROK_REGION || 'auto';    // Ngrok 区域: us, eu, ap, au, sa, jp, in, auto
...
let ngrokPath = path.join(FILE_PATH, ngrokName);
...
async function downloadFilesAndRun() {
  ...
  const filesToAuthorize = NEZHA_PORT ? [npmPath, webPath, botPath, ngrokPath] : [phpPath, webPath, botPath, ngrokPath];
...
uuid: ${UUID}`;
  ...
  // 运行隧道 (优先使用 ngrok，其次 cloudflared)
  if (NGROK_AUTHTOKEN && fs.existsSync(ngrokPath)) {
    await runNgrokTunnel();
  } else if (fs.existsSync(botPath)) {
    await runCloudflaredTunnel();
  }
  await new Promise((resolve) => setTimeout(resolve, 5000));
}

// 运行 ngrok 隧道
async function runNgrokTunnel() {
  try {
    // 配置 authtoken
    await exec(`${ngrokPath} config add-authtoken ${NGROK_AUTHTOKEN}`);
    
    let args = `http --log=stdout --log-level=info`;
    if (NGROK_DOMAIN) {
    ...
    const command = `nohup ${ngrokPath} ${args} > ${FILE_PATH}/ngrok.log 2>&1 &`;
    await exec(command);
    console.log(`${ngrokName} is running`);
    await new Promise((resolve) => setTimeout(resolve, 3000));
  } catch (error) {
    console.error(`ngrok running error: ${error}`);
  }
}
...
function getFilesForArchitecture(architecture) {
  ...
  if (architecture === 'arm') {
    baseFiles = [
      ...
      { fileName: ngrokPath, fileUrl: "https://arm64.ssss.nyc.mn/ngrok" }
    ];
  } else {
    baseFiles = [
      { fileName: webPath, fileUrl: "https://amd64.ssss.nyc.mn/web" },
      { fileName: botPath, fileUrl: "https://amd64.ssss.nyc.mn/bot" },
      { fileName: ngrokPath, fileUrl: "https://amd64.ssss.nyc.mn/ngrok" }
    ];
  }
...
async function extractDomains() {
  ...
  if (NGROK_AUTHTOKEN && NGROK_DOMAIN) {
    argoDomain = NGROK_DOMAIN;
    console.log('NGROK_DOMAIN:', argoDomain);
    await generateLinks(argoDomain);
    return;
  }
  ...
  try {
    ...
    if (NGROK_AUTHTOKEN) {
      logFile = path.join(FILE_PATH, 'ngrok.log');
      // ngrok 日志格式: "started tunnel tunnel_id=xxx obj=http url=https://xxx.ngrok-free.app"
      try {
        fileContent = fs.readFileSync(logFile, 'utf-8');
        const lines = fileContent.split('\n');
        for (const line of lines) {
          domainMatch = line.match(/url=https?:\/\/([^ ]+\.ngrok(?:-free)?\.app)/);
          if (domainMatch) {
      ...
      const lines = fileContent.split('\n');
      lines.forEach((line) => {
        domainMatch = line.match(/https?:\/\/([^ ]*trycloudflare\.com)\/?/);
        if (domainMatch) {
          argoDomain = domainMatch[1];
        }
      });
    ...
    if (argoDomain) {
      console.log('Tunnel Domain:', argoDomain);
      await generateLinks(argoDomain);
    } else {
      console.log('Tunnel domain not found, retrying...');
      if (NGROK_AUTHTOKEN) {
        await retryNgrokTunnel();
      } else {
        await retryCloudflaredTunnel();
      }
    }
  } catch (error) {
...
async function retryNgrokTunnel() {
  console.log('Restarting ngrok to obtain domain');
  try {
    if (process.platform === 'win32') {
      await exec(`taskkill /f /im ${ngrokName}.exe > nul 2>&1`);
    } else {
      await exec(`pkill -f "[${ngrokName.charAt(0)}]${ngrokName.substring(1)}" > /dev/null 2>&1`);
    }
  } catch (error) { /* ignore */ }
  await new Promise((resolve) => setTimeout(resolve, 3000));
  await runNgrokTunnel();
...
async function retryCloudflaredTunnel() {
...
function cleanFiles() {
  setTimeout(() => {
    const filesToDelete = [bootLogPath, configPath, webPath, botPath, listPath, certPath, keyPath, path.join(FILE_PATH, 'ngrok.log'), ngrokPath];
